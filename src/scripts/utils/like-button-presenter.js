...
const LikeButtonPresenter = {
  ...
  async _isRestaurantExist(id) {
    const restaurant = await this._favoriteRestos.getRestaurant(id);
    return !!restaurant;
  },
  ...
  _renderLike() {
    ...
    likeButton.addEventListener('click', async () => {
      await this._favoriteRestos.putRestaurant(this._restaurant);
      this._renderButton();
    });
  },
  ...
  _renderLiked() {
    ...
    const likeButton = document.querySelector('#likeButton');
    likeButton.addEventListener('click', async () => {
      await this._favoriteRestos.deleteRestaurant(this._restaurant.id);
      this._renderButton();
    });
  },
};
